...
Sudoku GASolver::Solve(std::vector<int> givens)
{
    ...
    vector<Sudoku> population, new_population;
    bool solved = false;
    srand(time(NULL));
    ...
    for(int iter = 0; iter < 1000000; iter++)
    {
        ...
        int restart = iter + 10000;

        //evolve
        for(;iter < restart; iter++)
        {
            //display evolution info
            if(iter % 100 == 0)
                cout << "Best fitness: " << population.back().Fitness << endl;
            ...
            while(new_population.size() != 2 * population.size())
            {
                ...
                new_population.push_back(
                    Sudoku(population[dad], population[mom]));
            }
