...
cv::Mat SudokuImage::DisplayHoughPlane()
{
    double max_val;
...
cv::Mat SudokuImage::DisplayRectifiedCells()
{
...
#pragma mark - Hough transform routines
...
void SudokuImage::CalcHoughLines()
{
    ...
    //Debug
    detected_img = input_image.clone();
    DrawLines(detected_img);
}


#pragma mark - Sudoku extraction routines
...
void SudokuImage::CalcPerspective()
{
    ...
    {
        circle(detected_img, intersections[i], 5, Scalar(0, 0, 255), -1);
    }
...
void SudokuImage::CalcCellCenters()
{
    ...
    for(int i = 10; i < 20 - 1; i++)
    {
        ...
        for(int j = 0; j < 10 - 1; j++)
        {
            ...
            //Debug
            circle(detected_img, p, 2, Scalar(255, 255, 0), -1);
        }
    }
...
//Constructor
SudokuImage::SudokuImage(cv::Mat image)
{
    //Read the image
    input_image = image;
