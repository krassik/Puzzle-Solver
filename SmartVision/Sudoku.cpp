...
Sudoku::Sudoku(const std::vector<int>& digits)
{
    ...
    for(int i = 0; i < SUDOKU_ORDER; i++)
    {
        ...
        {
            used[BlueprintChromosome[SUDOKU_ORDER*i + j]] = 1;
        }
        ...
        for(int j = 0; j < SUDOKU_ORDER; j++)
        {
            ...
            {
                Chromosome[SUDOKU_ORDER * i + j] =
                    BlueprintChromosome[SUDOKU_ORDER * i + j];
            }
        }
    }
...
void Sudoku::UpdatePhenotype()
{
    for(int i = 0; i < CHROMOSOME_LEN; i++)
    {
        Phenotype[i / SUDOKU_ORDER][i % SUDOKU_ORDER] = Chromosome[i];
    }
}
